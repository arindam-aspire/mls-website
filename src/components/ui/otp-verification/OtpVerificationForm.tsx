...
export function OtpVerificationForm({
  ...
  const getDigitClassName = (index: number) => {
    ...
    return cn(
      ...
      otpDigitTextClasses,
      hasValue && "border-primary bg-primary-light text-primary-dark",
      !hasValue && !isFocused && "border-secondary-light bg-surface text-text",
      isFocused && "border-primary bg-surface text-text ring-2 ring-primary/25",
    );
  };
    ...
    <form noValidate onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        >
          ...
          {digits.map((digit, index) => (
            <input
              ...
              maxLength={1}
              value={digit}
              aria-label={labels.digitLabel({ index: index + 1 })}
      ...
      <div className="space-y-2 text-center">
        ...
        {resendSeconds > 0 ? (
          <p
            className={cn(
              "inline-flex items-center justify-center gap-1.5 text-muted",
              bodyTextClasses,
            )}
          >
            <Clock className="size-4 shrink-0" aria-hidden />
            {labels.resendIn({ time: formatResendTimer(resendSeconds) })}
          </p>
        ) : null}
      </div>
      ...
      <Button
        ...
        disabled={!isComplete}
        isLoading={isLoading}
        loadingLabel={labels.confirmLoading}
