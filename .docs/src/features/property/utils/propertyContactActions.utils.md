...
- Card Email / Call / WhatsApp — `ensureListingAgentContactAllowed`: guests without a source agent open login; signed-in users without an agent continue so missing fields can toast.
...
| `ensureListingAgentContactAllowed` | Card Email/Call/WhatsApp — login when guest and source agent missing; allow signed-in users through |
...
4. Missing source agent on card payload → auth choose-account for **guests**; signed-in users proceed (missing email/phone toasts). Same gate for list card Email / Call / WhatsApp via `ensureListingAgentContactAllowed`.
