...
- **Below `md`:** primary row scrolls horizontally (scrollbar hidden); toggle **`8rem`**, other controls **`9rem`**; advanced filters open in a **bottom sheet** (not inline).
- **`md`–`lg`:** primary grid unchanged; advanced filters expand inline below the bar.
- **`lg`+:** eight-column primary row.
- Root wrapper: `relative isolate z-70` so dropdowns/advanced panel stack above list content (sticky bar on screen uses `z-30`).
