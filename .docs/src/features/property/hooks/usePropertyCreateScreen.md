...
- Build `PropertyForm` `config` via `buildPropertyFormConfig` (`propertyList.propertyCreate.form`, arrangement-aware identification fields).
- Pass the full property category taxonomy (Residential, Commercial, Land) into `PropertyForm` Category — no separate Properties/Land toggle. Derive arrangement from the selected category slug for the host DLS Location block: **Properties** (Residential/Commercial) shows Governate → Directorate → Village → Parcel Name → Parcel Number → Section → **Land Type** (`property_details.land_type_id`) → Plot Number → Building → Floor → Apartment; **Land** shows Governate → Directorate → Village → Parcel Name → Parcel Number → Section (dropdown) → Plot Number. Outbound `payload.location` is lat/lng + DLS + identification (`parcel_number`, `plot_number`, building/apartment, `identification_fields.floor_number`) plus `show_location`. Prunes identification keys that do not apply to the arrangement and clears `land_type_id` on Land during hydrate / arrangement change / Next / draft / submit. Catalog load syncs `propertyCategoriesRef` before hydrate so Land drafts prune correctly on first paint.
...
- When `GET /property-form-options` omits furnishing lists (or 404s), fill Furnishing Status from the existing search-filter master values so that dropdown still renders. Floor options are never filled — the Floor dropdown is hidden on Property Information.
...
- Expose `propertyFormContainerRef` and apply host DOM patches via [propertyCreateFormDom.utils.md](../utils/propertyCreateFormDom.utils.md): hide empty backend Reference Number and show it read-only once valued, hide Built-up Area unit (sqm only), rewrite owner document labels to **Owner ID or Passport**, hide library identification inputs (host DLS owns free-text fields; passes empty `visibleIdentificationKeys`).
...
- Own Location-step DLS selection (`gov_code` … `sect_name`) via `usePropertyLocationDls`, plus Land Type for Properties and host-owned identification text fields. Library step emissions are merged with the latest DLS selection the same way as `show_location`. Options load from `GET /dls-locations`. Host DLS / Land Type / identification API field errors stay off library `fieldErrors`.
...
- Land Type (`land_type_id` / `land_type`) is host-owned on `property_details` until `@abdoun/abdoun-library` adds it to `PropertyDetailsFormValues`. Helpers (`withPropertyFormLandTypeId`, prune-on-Land) type those fields via a local `PropertyDetailsWithLandType` intersection so TypeScript accepts writes without casting away the whole form.
