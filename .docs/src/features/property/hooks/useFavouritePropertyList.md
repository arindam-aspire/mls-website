...
- Expose `pageTitle`, `pageSubtitle`, `PropertyListingCardList` props: `listings`, `pagination`, `noDataFound`, handlers (no toolbar; grid layout only in screen).
- `cardButtonSize`: `"md"` for `registered_user` / `owner`; `"sm"` for other roles (`canTrackRecentPropertyView`).
- Contact actions use `usePropertyContactModalActions` (agent email/phone; owners are not used).
