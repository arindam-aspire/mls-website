...
- Account: profile link, Change Password, Notification Settings (`owner` / `registered_user` only). Preferences: Language, Theme Mode. Activity: **My Listings** (`owner`), **Manage Listings** (`admin` / `agency` / `agent`), Favourites, Saved Searches; **Recently Viewed** (`registered_user` / `owner`) — via `filterProfileMenuItemsWithRoleAccess`.
