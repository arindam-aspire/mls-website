...
Protected App Router pages under `(main)` call `useAuthorize(<permission>)` before rendering screens. Server edge guard: `proxy.ts` redirects unauthenticated requests (no `access_token` cookie) away from `/dashboard`, `/my-profile`, `/listing`, `/saved-searches`, `/favourites`, and `/recently-viewed`.
